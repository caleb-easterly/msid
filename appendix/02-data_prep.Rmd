...
reshape_pt_table <- function(tab) {
  ...
  tog <- rbind(m_only, w_only) %>%
    select(r_sex, r_sexid, r_sexact, rp_sex, pt_p, pt_p.se) %>%
...
all_sexid_pt_long %>%
  format_table()
use_data(all_sexid_pt_long, overwrite = TRUE)
```
...
all_sexid_props <- all_sexid_props %>%
  split_demo() %>%
  select(r_sex, r_sexid, r_sexact, prop_mean = mean, prop_se = SE)
...
sexid_tog_long %>%
  format_table()
use_data(sexid_tog_long, overwrite = TRUE)
```
...
het_sexid_long %>%
  format_table()
use_data(het_sexid_long, overwrite = TRUE)
```
