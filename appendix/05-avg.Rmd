...
avg_est <- all_sex %>%
    ...
    select(r_sex, r_sexact, m, msd, w, wsd)
...
fmtm <- avg_offered %>%
  ...
  mutate(rp_demo = paste(sex1, sexact1, sep="_"))
ggplot(fmtm) +
...
```{r}
make_partner_dist_avg <- function(report, dist){
left_join(report, dist,
        ...
        mutate(r_demo = paste(r_sex, r_sexact, sep="_"),
               rp_demo = paste(rp_sex, sexact1, sep="_")) %>%
  select(r_demo, rp_demo, prop, d_part_p)
}

avg_partner_dist <- make_partner_dist_avg(avg_rep, avg_offered)
...
men_bi_high <- avg_partner_dist %>%
  filter(r_demo == "m_high" & str_detect(rp_demo, "w_"))
...
avg_bal <- balance_het(avg_bidi)
format_table(avg_bal)

plot_mixing_matrix(avg_bal)
...
```{r}
all(with(avg_bal, abs(cnr - cnrp) < .Machine$double.eps), na.rm = TRUE)
```

### Compare
```{r}
sexid_nos_by_grp <- avg_bal %>%
